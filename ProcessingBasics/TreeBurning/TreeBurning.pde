 /**IMPORTANT:
  ...
  void setup() {
     ...
     *ANSWER HERE: The ratio of the width to the columns must equal the ratio of the height to the rows
     */
  ...
  void draw() {
     ...
     *ANSWER HERE: It slows down the proliferation by a factor of 10 by only changing the board every 10 frames - it helps us observe what's going on more clearly
     */
  ...
  void stringToSquares(String[]lines) {
    ...
    for(int i = 0; i < COLS; i++){
      for(int j = 0; j < ROWS; j++){
        // i = column, j = row
        String type = lines[j].substring(i, i + 1);
        switch (type){
          case "w":
            fill(255, 0, 0);
            break;
